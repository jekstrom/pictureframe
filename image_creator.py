...
class ImageCreator:
    ...
    def __init__(self, temperature, location, weather, todays_date, metric, s3_image_bucket):
    ...
    def draw_text(self, image):
        ...
        font_names = ["/var/task/fonts/segoe-ui.ttf", "segoeui.ttf", "segoe-ui.ttf", "/usr/share/fonts/truetype/ubuntu/Ubuntu-R.ttf"]
        heading = None
        subheading = None
        for font_name in font_names:
            try:
                heading = ImageFont.truetype(font_name, int(os.getenv("HEADING_FONT_SIZE")), encoding="unic")
                subheading = ImageFont.truetype(font_name, int(os.getenv("SUBHEADING_FONT_SIZE")), encoding="unic")
                break
    ...
    def getbuffer(self, image):
        ...
        if self.s3_image_bucket:
            filename = f"content/{self.location.replace(' ', '')}_{self.todays_date}.bmp"
